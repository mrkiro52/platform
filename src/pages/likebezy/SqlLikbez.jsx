    ...
    <div style={{ overflowX: 'auto', border: '1px solid var(--border-color)', borderRadius: name ? '0 8px 8px 8px' : 8 }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 'max-content' }}>
        <thead>
          <tr>
            {cols.map((c, j) => (
              <th key={j} style={{ padding: '8px 14px', textAlign: 'left', whiteSpace: 'nowrap', background: hCols.includes(j) ? 'rgba(255,214,10,0.18)' : 'var(--bg-secondary)', color: hCols.includes(j) ? 'var(--accent-lime)' : 'var(--text-secondary)', borderBottom: '2px solid var(--border-color)', fontFamily: 'monospace', fontWeight: 700 }}>{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} style={{ background: hRows.includes(i) ? 'rgba(255,214,10,0.09)' : 'transparent' }}>
              {row.map((cell, j) => (
...
DELETE FROM users WHERE age > 40;`} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10, margin: '16px 0' }}>
        {[
          ...
          { type: 'LEFT JOIN',  color: 'rgba(255,214,10,0.08)',   border: 'rgba(255,214,10,0.2)',   desc: 'Все строки слева + совпадения справа (NULL если нет)' },
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {[
            ...
            <div key={cmd} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ background: 'rgba(255,214,10,0.15)', color: 'var(--accent-lime)', borderRadius: 4, padding: '2px 7px', fontSize: 11, fontWeight: 700 }}>{num}</span>
