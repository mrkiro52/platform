...
print(pd.__version__)  # 2.x`} />
      ...
      <SelfCheck questions={[
        { q: 'Что такое Pandas и для чего он используется?', a: 'Pandas — библиотека Python для работы с табличными и временными данными. Она даёт удобные структуры (Series и DataFrame) и инструменты для загрузки, очистки, анализа и преобразования данных. Основа анализа данных и ML-пайплайнов.' },
        { q: 'Какие два главных объекта предоставляет Pandas?', a: 'Series — одномерный массив с индексом (как один столбец). DataFrame — двумерная таблица, набор Series с общим индексом (как лист Excel или таблица SQL).' },
        { q: 'Как принято импортировать Pandas?', a: 'import pandas as pd — стандартное сокращение, которое использует всё сообщество. Часто рядом импортируют numpy as np, так как они работают вместе.' },
      ]} />
...
s1 + s2  # a: NaN, b: 12, c: NaN`} />
      ...
      <SelfCheck questions={[
        { q: 'Что такое Series в Pandas?', a: 'Одномерный массив с метками-индексом. Можно думать о нём как о столбце таблицы или о словаре, где ключи стали индексом, а значения — данными.' },
        { q: 'Чем .loc[] отличается от .iloc[]?', a: ".loc[] обращается по метке индекса (s.loc['a']), .iloc[] — по числовой позиции (s.iloc[0]). При строковом индексе для доступа по позиции нужен именно .iloc[]." },
        { q: 'Что происходит при сложении двух Series с разными индексами?', a: 'Pandas выравнивает значения по индексу: совпадающие метки складываются, а для меток, которые есть только в одной серии, результат будет NaN.' },
      ]} />
...
df = pd.DataFrame(data, index=['r1', 'r2', 'r3'])`} />
      ...
      <SelfCheck questions={[
        { q: 'Что такое DataFrame?', a: 'Двумерная таблица данных: строки с общим индексом и именованные столбцы. Каждый столбец — это Series. Аналог таблицы в Excel или базе данных.' },
        { q: 'Чем отличается df["col"] от df[["col"]]?', a: 'df["col"] возвращает один столбец как Series. df[["col"]] с двойными скобками возвращает DataFrame (можно передать список из нескольких столбцов).' },
        { q: 'Как из DataFrame получить только определённые столбцы?', a: 'Передать список их имён: df[["name", "age"]]. Порядок в списке задаёт порядок столбцов в результате.' },
      ]} />
...
df.to_sql('table_name', conn, if_exists='replace', index=False)`} />
      ...
      <SelfCheck questions={[
        { q: 'Каким методом прочитать CSV-файл в DataFrame?', a: 'pd.read_csv("file.csv"). Для Excel — pd.read_excel(), для JSON — pd.read_json(), для SQL — pd.read_sql(). У read_csv много параметров: sep, encoding, usecols и др.' },
        { q: 'Как сохранить DataFrame в CSV без столбца индекса?', a: 'df.to_csv("file.csv", index=False). Параметр index=False убирает технический числовой индекс из файла, оставляя только данные.' },
        { q: 'Что делать, если CSV использует другой разделитель, например ";"?', a: 'Указать параметр sep: pd.read_csv("file.csv", sep=";"). Так часто оформляют файлы из европейского Excel, где запятая — десятичный знак.' },
      ]} />
...
df.memory_usage(deep=True)  # память в байтах`} />
      ...
      <SelfCheck questions={[
        { q: 'Какие методы используют при первом знакомстве с данными?', a: 'df.head() — первые строки, df.info() — типы и пропуски, df.describe() — статистика числовых столбцов. Это стандартный первый шаг любого анализа.' },
        { q: 'Что показывает df.info()?', a: 'Список столбцов, число непустых значений в каждом, типы данных (dtype) и объём занимаемой памяти. Помогает сразу увидеть пропуски и неверные типы.' },
        { q: 'Как быстро узнать число уникальных значений в столбце?', a: 'df["col"].nunique() — количество уникальных значений, df["col"].value_counts() — частоту каждого значения. Полезно для категориальных признаков.' },
      ]} />
...
df.reindex(columns=['age', 'name', 'score', 'city'])`} />
      ...
      <SelfCheck questions={[
        { q: 'Чем .loc[] отличается от .iloc[] в DataFrame?', a: '.loc[строка, столбец] обращается по меткам, .iloc[позиция, позиция] — по числовым индексам. Для фильтрации по условию используют .loc: df.loc[df["age"] > 18].' },
        { q: 'Как выбрать конкретные строки и столбцы одновременно?', a: 'df.loc[2:4, ["name", "age"]] — по меткам строк и именам столбцов, либо df.iloc[2:5, 0:2] — по числовым позициям. .loc включает правую границу, .iloc — нет.' },
        { q: 'Почему рекомендуют использовать .loc/.iloc вместо цепочки []?', a: 'Цепочка вида df[df.a>0]["b"] = 1 может менять копию, а не оригинал (SettingWithCopyWarning). .loc[] делает явную и безопасную выборку и присваивание.' },
      ]} />
...
df['score'].where(df['score'] >= 80)`} />
      ...
      <SelfCheck questions={[
        { q: 'Как отфильтровать строки по условию age > 25?', a: 'df[df["age"] > 25]. Внутри создаётся булева маска (True/False для каждой строки), и остаются только строки со значением True.' },
        { q: 'Как объединить несколько условий фильтрации?', a: 'Каждое условие в скобках, между ними & (и), | (или), ~ (не): df[(df["age"] > 25) & (df["city"] == "Москва")]. Обычные and/or здесь не работают.' },
        { q: 'Что такое булева маска?', a: 'Series из True/False той же длины, что и DataFrame. При передаче в df[mask] остаются строки, где True. Маску создаёт условие: df["age"] > 18.' },
      ]} />
...
df.replace({'city': {'Msk': 'Moscow', 'Spb': 'SPb'}})`} />
      ...
      <SelfCheck questions={[
        { q: 'Как обозначаются пропущенные значения в Pandas?', a: 'Как NaN (Not a Number). Найти их помогают df.isnull() и df.notnull(), а df.isnull().sum() покажет число пропусков в каждом столбце.' },
        { q: 'Чем fillna() отличается от dropna()?', a: 'fillna(value) заменяет NaN на значение (0, среднее, "неизвестно"), сохраняя строки. dropna() удаляет строки или столбцы с пропусками. fillna безопаснее — не теряет данные.' },
        { q: 'Как заполнить пропуски средним значением столбца?', a: 'df["col"].fillna(df["col"].mean()). Для категориальных данных чаще используют моду или строку-заглушку вместо среднего.' },
      ]} />
...
df = (df
    ...
    .assign(grade=lambda x: x['score'].apply(
      ...
      <SelfCheck questions={[
        { q: 'Как создать новый столбец на основе существующих?', a: 'Присвоить выражение: df["total"] = df["price"] * df["qty"]. Можно использовать арифметику над столбцами или apply для сложной логики.' },
        { q: 'Как переименовать столбцы?', a: 'df.rename(columns={"old": "new"}) переименует выбранные, а df.columns = [...] задаст все имена сразу. rename не меняет оригинал без inplace=True.' },
        { q: 'Как удалить столбец из DataFrame?', a: 'df.drop(columns=["col"]) или df.drop("col", axis=1). Чтобы изменить сам объект, добавляют inplace=True либо переприсваивают результат.' },
      ]} />
...
pd.api.types.is_string_dtype(df['name'])     # True`} />
      ...
      <SelfCheck questions={[
        { q: 'Зачем приводить столбцы к правильным типам?', a: 'Неверный тип замедляет работу и ломает операции: числа-строки нельзя суммировать, даты-строки нельзя сравнивать как даты. Правильные типы экономят память и ускоряют вычисления.' },
        { q: 'Как преобразовать столбец к числовому типу?', a: 'pd.to_numeric(df["col"], errors="coerce"). Параметр errors="coerce" превращает непарсируемые значения в NaN вместо ошибки. Для общего приведения есть df["col"].astype(int).' },
        { q: 'Что даёт тип category для строковых столбцов?', a: 'Если строки часто повторяются (пол, город, статус), тип category хранит их как коды, экономя память и ускоряя группировку и сравнение.' },
      ]} />
...
df['rank'] = df['score'].rank(ascending=False, method='min')`} />
      ...
      <SelfCheck questions={[
        { q: 'Как отсортировать DataFrame по столбцу?', a: 'df.sort_values("age") — по возрастанию, df.sort_values("age", ascending=False) — по убыванию. Можно сортировать по нескольким столбцам списком.' },
        { q: 'Как сортировать по двум столбцам в разном порядке?', a: 'df.sort_values(["city", "age"], ascending=[True, False]) — сначала по городу по возрастанию, внутри города по возрасту по убыванию.' },
        { q: 'Чем sort_values отличается от sort_index?', a: 'sort_values сортирует по значениям в столбцах, sort_index — по меткам индекса. Первый используют для упорядочивания данных, второй — для упорядочивания по индексу.' },
      ]} />
...
df.groupby('city').filter(lambda x: x['score'].mean() > 85)`} />
      ...
      <SelfCheck questions={[
        { q: 'Что делает df.groupby("city")["salary"].mean()?', a: 'Группирует строки по уникальным значениям city и для каждой группы считает среднюю зарплату. Результат — Series: город → среднее значение.' },
        { q: 'Как применить несколько агрегаций сразу?', a: 'df.groupby("city")["salary"].agg(["mean", "max", "count"]) или именованные агрегации .agg(avg=("salary","mean"), total=("salary","sum")).' },
        { q: 'Что такое принцип split-apply-combine?', a: 'Логика groupby: данные разбиваются на группы (split), к каждой применяется функция (apply), результаты собираются в итоговую таблицу (combine).' },
      ]} />
...
df['score'] * 2`} />
      ...
      <SelfCheck questions={[
        { q: 'Когда использовать apply() вместо векторных операций?', a: 'apply() медленнее, поэтому его берут только для сложной логики, которую нельзя выразить векторно (через +, *, str-методы). Где возможно — векторные операции быстрее.' },
        { q: 'Чем map() отличается от apply()?', a: 'map() работает только с Series и подходит для простых поэлементных преобразований. apply() работает и с Series, и со строками/столбцами DataFrame (через axis).' },
        { q: 'Что делает axis=1 в df.apply(func, axis=1)?', a: 'Функция применяется к каждой строке целиком (а не к столбцу). Это позволяет вычислять значение на основе нескольких столбцов одной строки.' },
      ]} />
...
df1.join(df2, on='city')  # из df1 использовать столбец city`} />
      ...
      <SelfCheck questions={[
        { q: 'Чем merge() отличается от concat()?', a: 'merge() соединяет таблицы по ключевому столбцу (как SQL JOIN). concat() просто склеивает таблицы по строкам или столбцам без сопоставления ключей.' },
        { q: 'Что делает параметр how в merge()?', a: 'Задаёт тип соединения: "inner" (только совпадения), "left" (все из левой), "right" (все из правой), "outer" (все строки обеих таблиц). По умолчанию inner.' },
        { q: 'Как объединить два DataFrame по вертикали (друг под другом)?', a: 'pd.concat([df1, df2]) с axis=0 (по умолчанию). Столбцы сопоставляются по именам; ignore_index=True пересоберёт индекс заново.' },
      ]} />
...
s.str.zfill(5)                  # дополнить нулями слева`} />
      ...
      <SelfCheck questions={[
        { q: 'Как применить строковый метод ко всему столбцу?', a: 'Через аксессор .str: df["name"].str.lower(), .str.strip(), .str.upper(). Он применяет операцию к каждому значению столбца сразу.' },
        { q: 'Как проверить, содержит ли строка подстроку?', a: 'df["city"].str.contains("Моск") возвращает булеву Series. Её используют для фильтрации: df[df["city"].str.contains("Моск")].' },
        { q: 'Как разбить строку на части по разделителю?', a: 'df["fio"].str.split(" ") вернёт списки, а с параметром expand=True — отдельные столбцы. Удобно для разбора ФИО, адресов и т.п.' },
      ]} />
...
df['score'].diff()     # разница с предыдущей строкой`} />
      ...
      <SelfCheck questions={[
        { q: 'Как преобразовать столбец со строками-датами в тип datetime?', a: 'pd.to_datetime(df["date"]). После этого становится доступен аксессор .dt для работы с компонентами даты и времени.' },
        { q: 'Как выделить год, месяц и день из даты?', a: 'Через аксессор .dt: df["date"].dt.year, .dt.month, .dt.day. Также доступны .dt.hour, .dt.dayofweek и другие компоненты.' },
        { q: 'Зачем устанавливать дату как индекс DataFrame?', a: 'С DatetimeIndex доступны срезы по датам (df["2024-01"]), ресемплинг (resample) и удобная работа с временными рядами — группировка по дням, месяцам, годам.' },
      ]} />
...
df_long = df.melt(
    ...
    value_name='value',
      ...
      <SelfCheck questions={[
        { q: 'Чем pivot_table() отличается от pivot()?', a: 'pivot() просто переставляет данные и требует уникальных комбинаций. pivot_table() умеет агрегировать (sum, mean) и корректно обрабатывает дубликаты.' },
        { q: 'Что задают параметры index, columns и values в pivot_table?', a: 'index — что станет строками, columns — что станет столбцами, values — какие значения агрегировать. aggfunc определяет функцию агрегации (по умолчанию mean).' },
        { q: 'Что делает параметр margins=True?', a: 'Добавляет строку и столбец "All" с итогами по всем группам — удобно для сводных отчётов с общими суммами или средними.' },
      ]} />
...
df['score'].cummax()   # нарастающий максимум`} />
      ...
      <SelfCheck questions={[
        { q: 'Что такое скользящее среднее и как его посчитать?', a: 'Среднее за последние N значений. df["col"].rolling(window=7).mean() даёт скользящее среднее за 7 периодов — сглаживает колебания во временных рядах.' },
        { q: 'Чем rolling() отличается от expanding()?', a: 'rolling(N) использует фиксированное окно из N последних значений. expanding() использует окно от начала до текущей строки, которое растёт. expanding().mean() — накопленное среднее.' },
        { q: 'Что делает метод shift()?', a: 'Сдвигает значения столбца вверх или вниз: df["col"].shift(1) даёт значение предыдущей строки. Используется для расчёта изменений и сравнения с прошлым периодом.' },
      ]} />
...
def optimize_dtypes(df):
    ...
    return df`} />
      ...
      <SelfCheck questions={[
        { q: 'Почему стоит избегать iterrows() и циклов по строкам?', a: 'iterrows() очень медленный — обходит строки в Python-цикле и теряет преимущества векторизации NumPy. Лучше использовать векторные операции, apply или np.where.' },
        { q: 'Как уменьшить расход памяти большого DataFrame?', a: 'Приводить числовые столбцы к меньшим типам (int32, float32) через astype, а повторяющиеся строки — к category. df.info(memory_usage="deep") покажет реальный размер.' },
        { q: 'Что быстрее: векторная операция или apply?', a: 'Векторная операция (df["a"] + df["b"]) почти всегда быстрее, так как выполняется на уровне NumPy в C. apply работает поэлементно в Python и медленнее.' },
      ]} />
      ...
      <SelfCheck questions={[
        { q: 'Какие три метода всегда применяют при первом взгляде на данные?', a: 'df.head() — первые строки, df.info() — типы и пропуски, df.describe() — статистика. Это базовый старт любого разведочного анализа (EDA).' },
        { q: 'Какой метод покажет частоту значений в столбце?', a: 'df["col"].value_counts() — частоту каждого уникального значения по убыванию. С normalize=True вернёт доли вместо абсолютных чисел.' },
        { q: 'Чем фильтрация по маске отличается от .query()?', a: 'Маска: df[df["age"] > 18]. query() позволяет писать условие строкой: df.query("age > 18 and city == \'Москва\'"). query короче и читабельнее для сложных условий.' },
      ]} />
