...
const Note = ({ children }) => (
  <div style={{ background: 'rgba(200,255,0,0.05)', border: '1px solid rgba(200,255,0,0.18)', borderRadius: 8, padding: '12px 16px', margin: '14px 0', color: 'var(--text-secondary)', fontSize: 13.5, lineHeight: 1.7 }}>
  ...
  return (
    <VizBox title="Градиентный спуск по шагам">
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: 460, background: 'var(--bg-tertiary)', borderRadius: 8, display: 'block' }}>
        <line x1={pad} y1={H - pad} x2={W - pad} y2={H - pad} stroke="var(--border-color)" strokeWidth="1.5" />
        <line x1={sx(3)} y1={pad} x2={sx(3)} y2={H - pad} stroke="rgba(200,255,0,0.25)" strokeWidth="1" strokeDasharray="4 3" />
    ...
    <VizBox title="ROC-кривая и порог классификатора">
      <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center' }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: 300, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
          {/* AUC area */}
          <path d={area} fill="rgba(200,255,0,0.1)" />
          {/* diagonal random */}
