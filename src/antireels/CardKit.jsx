...
export function Formula({ children }) {
  return (
    <div style={{
      ...
      fontFamily: 'var(--font-mono, monospace)', fontSize: 'clamp(14px, 3.8vw, 17px)',
...
export function Code({ children }) {
  return (
    <pre style={{
      ...
      borderRadius: 10, padding: '14px 16px', margin: 0, overflowX: 'auto',
      fontFamily: 'var(--font-mono, monospace)', fontSize: 'clamp(12.5px, 3.4vw, 14.5px)',
...
export function Ul({ items }) {
  return (
    <ul style={{ margin: 0, paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 7 }}>
      {items.map((it, i) => (
        <li key={i} style={{ color: 'var(--text-secondary)', fontSize: 'clamp(14px, 3.6vw, 15.5px)', lineHeight: 1.55 }}>{it}</li>
      ))}
    </ul>
  )
}
...
export function Fig({ children, caption }) {
  return (
    <figure style={{ margin: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      ...
      {caption && (
        <figcaption style={{ color: 'var(--text-tertiary)', fontSize: 12.5, textAlign: 'center' }}>{caption}</figcaption>
      )}
    </figure>
  )
}
