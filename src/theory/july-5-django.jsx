  ...
  return (
    <div className="theory-container">
      ...
      <section className="theory-section">
        ...
        <TheoryCode language="bash" code={`# 1. Устанавливаем зависимости
pip install django djangorestframework

# 2. Создаём проект и приложение
django-admin startproject config .
python manage.py startapp library`} />
        <p>Подключаем DRF и наше приложение в настройках проекта:</p>
        <TheoryCode language="python" code={`# config/settings.py
INSTALLED_APPS = [
...
class Book(models.Model):
    title = models.CharField(max_length=200)      # название (строка)
    author = models.CharField(max_length=100)     # автор
    year = models.IntegerField()                  # год издания (число)
    is_read = models.BooleanField(default=False)  # прочитана ли (да/нет)
    ...
    def __str__(self):
        ...
        <TheoryCode language="bash" code={`python manage.py makemigrations   # создать файл миграции
python manage.py migrate           # применить: создать таблицу в БД`} />
      </section>
...
class BookSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        # или перечислить явно: ['id', 'title', 'author', 'year', 'is_read']`} />
...
class BookViewSet(viewsets.ModelViewSet):
    queryset = Book.objects.all()        # какие записи обслуживаем
    serializer_class = BookSerializer    # чем сериализуем

# Всё! Этот класс уже умеет GET / POST / PUT / PATCH / DELETE`} />
      </section>
      ...
      <section className="theory-section">
        ...
        <TheoryCode language="python" code={`# config/urls.py
from django.urls import path, include
from rest_framework.routers import DefaultRouter
from library.views import BookViewSet

router = DefaultRouter()
router.register(r'books', BookViewSet)   # ресурс "books"
...
urlpatterns = [
    path('api/', include(router.urls)),
      ...
      <section className="theory-section">
        ...
        <TheoryCode language="bash" code={`python manage.py runserver
# Сервер поднимется на http://127.0.0.1:8000/`} />
        ...
        <TheoryCode language="bash" code={`# Создать книгу (POST)
curl -X POST http://127.0.0.1:8000/api/books/ \\
  -H "Content-Type: application/json" \\
  -d '{"title": "1984", "author": "Оруэлл", "year": 1949}'

# Получить список (GET)
curl http://127.0.0.1:8000/api/books/

# Обновить (PATCH) — отметить прочитанной
