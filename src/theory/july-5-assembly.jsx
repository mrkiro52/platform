  ...
  return (
    <div className="theory-container">
        ...
        <Fig caption="Лестница абстракции: от удобного для человека Python вниз, к машинному коду, который непосредственно исполняет процессор. Ассемблер — последняя ступень, ещё читаемая человеком">
          <svg viewBox="0 0 560 250" width="100%" style={{ maxWidth: 560 }} xmlns="http://www.w3.org/2000/svg">
            {[
              ...
              { t: 'Ассемблер (NASM)', d: 'мнемоники машинных команд', c: '#c8ff00', y: 132 },
      ...
      <section className="theory-section">
        ...
        <Fig caption="Модель исполнения: процессор с регистрами тянет данные из RAM, обрабатывает в регистрах и кладёт обратно. RIP всегда указывает на следующую команду">
          <svg viewBox="0 0 580 200" width="100%" style={{ maxWidth: 580 }} xmlns="http://www.w3.org/2000/svg">
            <rect x="30" y="30" width="230" height="140" rx="10" fill="rgba(200,255,0,0.05)" stroke={C.lime} />
      ...
      <section className="theory-section">
        ...
        <TheoryCode language="asm" code={`section .data
    msg db "Hello, World", 10     ; строка + 10 (символ перевода строки)
    len equ $ - msg               ; длина строки (текущий адрес минус адрес msg)

section .text
    global _start                 ; точка входа

_start:
    ; --- системный вызов write(fd=1, buf=msg, count=len) ---
    mov rax, 1        ; номер syscall: 1 = write
    mov rdi, 1        ; аргумент 1: fd = 1 (стандартный вывод, экран)
    mov rsi, msg      ; аргумент 2: адрес строки
    mov rdx, len      ; аргумент 3: сколько байт вывести
    syscall           ; вызвать ядро → строка печатается

    ; --- системный вызов exit(0) — корректно завершить программу ---
    mov rax, 60       ; номер syscall: 60 = exit
    mov rdi, 0        ; код возврата 0 (успех)
    syscall`} />
        <p>Собрать и запустить (нужен ассемблер NASM и компоновщик ld):</p>
        <TheoryCode language="bash" code={`nasm -f elf64 hello.asm -o hello.o   # ассемблировать в объектный файл
ld hello.o -o hello                  # скомпоновать в исполняемый
./hello                              # запустить → Hello, World`} />
      ...
      <section className="theory-section">
        ...
        <TheoryCode language="asm" code={`    mov rax, 0        ; сумма = 0
    mov rcx, 1        ; счётчик i = 1

loop_start:
    add rax, rcx      ; сумма += i
    inc rcx           ; i++
    cmp rcx, 6        ; сравнить i с 6
    jl loop_start     ; если i < 6 — прыгнуть в начало цикла

    ; после цикла rax = 1+2+3+4+5 = 15`} />
        <Fig caption="Цикл в ассемблере: тело выполняется, счётчик растёт, cmp сравнивает его с границей, а условный переход jl либо возвращает в начало, либо выпускает из цикла">
          <svg viewBox="0 0 560 180" width="100%" style={{ maxWidth: 560 }} xmlns="http://www.w3.org/2000/svg">
            <rect x="40" y="70" width="120" height="40" rx="8" fill="rgba(200,255,0,0.10)" stroke={C.lime} />
